...
export async function updateUserProfile(form) {
  try {
    ...
    let avatarUrl = "";
    if (form.avatar) {
      avatarUrl = await uploadFile(form.avatar, "image");
      if (avatarUrl !== "") {
    ...
    if (form.username !== "") {
      dataToUpdate["username"] = form.username;
    }

    const updatedUser = await databases.updateDocument(
      config.databaseId, // ID of the Appwrite database
      config.userCollectionId, // ID of the collection to update
      form.userId, // document id
      dataToUpdate
    );
    return updatedUser;
  } catch (error) {
...
// TODO: DELETE FILE ON AVATAR CHANGE
// STORE AVATAR FILE ID IN DB